...
{
  NigeruTestInit(manager);
  NigeruInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("NigeruTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("NigeruTest0");
  if (comp == nullptr)
  {
  ...
  NigeruTest* testcomp = dynamic_cast<NigeruTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
